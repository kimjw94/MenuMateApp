<template lang="html">
      ...
      <div class="signup-card">
        ...
        <form @submit.prevent="signup" class="signup-form">
          ...
          <div class="form-group" :class="{ 'has-error': !valid.userPassword }">
            ...
            <div class="error-message-container">
              <span v-if="!valid.userPassword" class="error-message">비밀번호는 6자 이상이어야 합니다.</span>
            </div>
          </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        phoneAreas: ['010', '02', '031', '032', '033', '041', '042', '043', '044', '051', '052', '053', '054', '055', '061', '062', '063', '064'], // 지역번호 배열
        valid: {
          userId: true,
          userPassword: true,
    ...
    methods: {
      ...
      validatePassword() {
        this.valid.userPassword = this.user.userPassword.length >= 6;
      },
      validatePasswordChk() {
  ...
  .signup-container {
    ...
    height: 80vh; /* 높이를 줄여 상단에 더 가까이 위치하도록 조정 */
    padding-top: 15px; /* 여유 공간 추가 */
  ...
  .form-group input,
  .form-group select {
    flex: 1; /* 입력 필드와 셀렉트 박스의 너비를 균등하게 */
    padding: 8px;
...
.phone-group input {
    width: calc(32% - 5px); /* 여백을 고려한 너비 조정 */
}
